...
export const pagesRouter: RouteHandler = async (request, env) => {
  ...
  if (request.method === 'POST' && parts[1] === 'submit') {
    ...
    await dbRun(env.DB, `UPDATE landing_pages SET submissions=submissions+1,${touchUpdated()} WHERE id=?`, [String(p.id)]);

    // Insert lead
    await dbRun(env.DB,
      `INSERT INTO leads (campaign_id,client_id,first_name,last_name,email,phone,company,title,industry,company_size,country,asset_downloaded,consent_flag,status,source)
      ...
      [String(p.campaign_id), String(p.client_id), body.first_name??'', body.last_name??'',
       ...
       body.industry??null, body.company_size??null, body.country??null, p.name]
    );
  ...
  if (request.method === 'POST') {
    ...
    const result = await dbRun(env.DB,
      `INSERT INTO landing_pages (campaign_id,client_id,asset_id,name,slug,headline,subheadline,cta_text,fields,status)
       VALUES (?,?,?,?,?,?,?,?,?,?)`,
  ...
  if (request.method === 'PUT' && id) {
    const b: any = await request.json();
    await dbRun(env.DB,
      `UPDATE landing_pages SET name=?,slug=?,headline=?,subheadline=?,cta_text=?,fields=?,status=?,${touchUpdated()} WHERE id=?`,
      [b.name, b.slug, b.headline??null, b.subheadline??null, b.cta_text??'Download Now',
       b.fields ? JSON.stringify(b.fields) : '["first_name","last_name","email","company","title","phone"]',
       b.status??'draft', String(id)]
    );
